...
public class Test7192955 {
    ...
    public static void main(String[] args) throws IntrospectionException {
        if (!CompactBeanUtils.findPropertyDescriptor(MyBean.class, "test").isBound()) {
            throw new Error("a simple property is not bound");
        }
        if (!CompactBeanUtils.findPropertyDescriptor(MyBean.class, "list").isBound()) {
            throw new Error("a generic property is not bound");
        }
        if (!CompactBeanUtils.findPropertyDescriptor(MyBean.class, "readOnly").isBound()) {
