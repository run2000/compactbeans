...
public class Test4619536 {
    public static void main(String[] args) throws Exception {
        IndexedPropertyDescriptor ipd = CompactBeanUtils.getIndexedPropertyDescriptor(A.class, "foo");
        if (!ipd.getIndexedPropertyType().equals(String.class)) {
            error(ipd, "A.foo should be String type");
        }
        PropertyDescriptor pd = CompactBeanUtils.findPropertyDescriptor(B.class, "foo");
        ...
        pd = CompactBeanUtils.findPropertyDescriptor(Child.class, "foo");
        if (pd instanceof IndexedPropertyDescriptor) {
            error(pd, "Child.foo should not be an indexed property");
        }
        pd = CompactBeanUtils.findPropertyDescriptor(Classic.class, "foo");
        if (pd instanceof IndexedPropertyDescriptor) {
            error(pd, "Classic.foo should not be an indexed property");
        }
        ipd = CompactBeanUtils.getIndexedPropertyDescriptor(Index.class, "foo");
        if (!hasIPD(ipd)) {
        ...
        ipd = CompactBeanUtils.getIndexedPropertyDescriptor(All.class, "foo");
        ...
        ipd = CompactBeanUtils.getIndexedPropertyDescriptor(Getter.class, "foo");
        ...
        ipd = CompactBeanUtils.getIndexedPropertyDescriptor(BadGetter.class, "foo");
        if (hasPD(ipd)) {
            error(ipd, "BadGetter.foo should not have classic methods");
        }
        ipd = CompactBeanUtils.getIndexedPropertyDescriptor(Setter.class, "foo");
        if (ipd.getReadMethod() != null || ipd.getWriteMethod() == null) {
        ...
        ipd = CompactBeanUtils.getIndexedPropertyDescriptor(BadSetter.class, "foo");
        if (hasPD(ipd)) {
