 ...
 * @jdksource com.sun.beans.util.Cache
 */
final class MethodCache {
     ...
     *
     * @param signature a key that can be used to create a value
    ...
    public Method create(Signature signature) {
        try {
            ...
            return MethodFinder.findAccessibleMethod(finder.find(signature.getType().getMethods()));
        }
     ...
     */
    public MethodCache(Kind keyKind, Kind valueKind) {
     ...
     * @throws NullPointerException if {@code keyKind} or {@code valueKind} are {@code null}
     */
    public MethodCache(Kind keyKind, Kind valueKind, boolean identity) {
     ...
     */
    public final Method get(Signature key) {
        ...
        Method current = getEntryValue(key, hash, table[index(hash, table)]);
        if (current != null) {
            return current;
        }
     ...
     */
    public final void remove(Signature key) {
        if (key != null) {
            synchronized (this.queue) {
                removeStaleEntries();
                int hash = hash(key);
    ...
    private Method getEntryValue(Signature key, int hash, CacheEntry entry) {
        while (entry != null) {
            if (entry.matches(hash, key)) {
    ...
    private final class CacheEntry {
        ...
        private final Ref<Signature> key;
        private final Ref<Method> value;
         ...
         */
        private CacheEntry(int hash, Signature key, Method value, CacheEntry next) {
            this.hash = hash;
            this.key = MethodCache.this.keyKind.create(this, key, MethodCache.this.queue);
            this.value = MethodCache.this.valueKind.create(this, value, MethodCache.this.queue);
            this.next = next;
        }
        ...
        private boolean matches(int hash, Object object) {
            ...
            return (key == object) || !MethodCache.this.identity && (key != null) && key.equals(object);
        }
    ...
    static enum Kind {
        STRONG {
            <T> Ref<T> create(Object owner, T value, ReferenceQueue<? super T> queue) {
                return new Kind.Strong<T>(owner, value);
            }
        },
        SOFT {
            <T> Ref<T> create(Object owner, T referent, ReferenceQueue<? super T> queue) {
                return (referent == null)
                        ? new Kind.Strong<T>(owner, referent)
                        : new Kind.Soft<T>(owner, referent, queue);
            }
        },
        WEAK {
            <T> Ref<T> create(Object owner, T referent, ReferenceQueue<? super T> queue) {
                return (referent == null)
                        ? new Kind.Strong<T>(owner, referent)
                        : new Kind.Weak<T>(owner, referent, queue);
            }
        };
        ...
        /**
         * This is an implementation of the {@link Ref} interface
